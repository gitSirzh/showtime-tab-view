...
import Animated, { cancelAnimation, Extrapolation, interpolate, runOnJS, useAnimatedReaction, useAnimatedStyle, useDerivedValue, useSharedValue, withDecay, withTiming } from "react-native-reanimated";
...
export const GestureContainer = /*#__PURE__*/React.forwardRef(function GestureContainer(_ref, forwardedRef) {
  ...
  const headerTransValue = useDerivedValue(() => {
    ...
    if (isIOS && !isDragging.value && !isRefreshing.value && !isRefreshingWithAnimation.value && !disableBounces.value && shareAnimatedValue.value < 0 && animatedScrollableState.value === SCROLLABLE_STATE.UNLOCKED) {
      return -shareAnimatedValue.value;
    }
