...
import type { TabViewProps, Route as TabViewRoute } from "react-native-tab-view";
import type { SCROLLABLE_STATE } from "./constants";
export type Route = TabViewRoute & {
    index: number;
    subtitle?: string | number;
};
export declare enum RefreshTypeEnum {
    Idle = 0,
    Pending = 1,
    Success = 2,
    Refreshing = 3,
...
export type GestureContainerProps<T extends Route> = Pick<TabViewProps<Route>, "navigationState"> & CollapsibleHeaderProps<T> & {
    initialPage: number;
